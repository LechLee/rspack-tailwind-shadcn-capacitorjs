									...
									<Tooltip key={theme.name}>
										<TooltipTrigger asChild>
											<button
												...
												className={cn('flex size-8 items-center justify-center rounded-full border-2 text-xs', isActive ? 'border-[--theme-primary]' : 'border-transparent')}
												style={{
													'--theme-primary': `hsl(${theme?.activeColor[mode === 'dark' ? 'dark' : 'light']})`
												}}
											>
												<span className={cn('flex size-5 items-center justify-center rounded-full bg-[--theme-primary]')}>{isActive && <CheckIcon className="size-4 text-white" />}</span>
					...
					<div className="flex w-full items-center">
						...
						<Popover>
							...
							<PopoverContent className="space-y-3 rounded-[0.5rem] text-sm" side="right" align="start" alignOffset={-20}>
								...
								<p>
									The <span className="font-medium">Default</span> style has larger inputs, uses lucide-react for icons and tailwindcss-animate for animations.
								</p>
								<p>
									The <span className="font-medium">New York</span> style ships with smaller buttons and cards with shadows. It uses icons from Radix Icons.
								</p>
							</PopoverContent>
						</Popover>
					</div>
					<div className="grid grid-cols-3 gap-2">
						<Button variant="outline" size="sm" onClick={() => setConfig({ ...config, style: 'new-york' })} className={cn(config.style === 'new-york' && 'border-2 border-primary')}>
							New York
						</Button>
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<Button
										...
										className={cn('opacity-50', config.style === 'default' && 'border-2 border-primary')}
									>
										Default
									</Button>
								</TooltipTrigger>
					...
					<div className="grid grid-cols-3 gap-2">
						{themes.map((theme) => {
							...
							return mounted ? (
								...
								>
									<span className={cn('mr-1 flex size-5 shrink-0 -translate-x-1 items-center justify-center rounded-full bg-[--theme-primary]')}>
