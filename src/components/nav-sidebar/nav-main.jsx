	...
	return (
		<SidebarGroup>
			<div className="flex items-center justify-between">
				...
				>
					{items.every((item) => openItems[item.title]) ? <ChevronsDownUp className="size-4" /> : <ChevronsUpDown className="size-4" />}
				</Button>
			</div>
			<SidebarMenu>
				{items.map((item) => (
					<Collapsible key={item.title} asChild open={openItems[item.title]} onOpenChange={() => handleToggle(item.title)}>
						<SidebarMenuItem>
							<SidebarMenuButton asChild tooltip={t(item.title)} data-active={item.children ? false : isPathActive(item.to)}>
								<Link to={item.to}>
									<item.icon />
										...
										<SidebarMenuSub>
											{item.children?.map((subItem) => (
												<SidebarMenuSubItem key={subItem.title}>
													<SidebarMenuSubButton asChild data-active={isPathActive(subItem.to)}>
														<Link to={subItem.to}>
